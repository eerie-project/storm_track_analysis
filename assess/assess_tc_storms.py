...
def get_model_storms(runid_info, yearstart, yearend, yearsplot, dir_in_base, plot_dir = '', coupled=True):
    ...
    for ir, suiteid in enumerate(runid_info['model']):
        for ia, algo in enumerate(runid_info['algo_type'][ir:ir+1]):
            ...
            key = tc_calculate.define_key(algo, expt, ens)
...
def work(runid_info, yearstart, yearend, yearsplot, dir_in_base, plot_dir = '', apply_threshold = False, coupled=True, storedir='./', do_meanstate=True, do_variability=True):
    ...
    tc_calculate.calculate_tc_properties(runid_info, storms, years, storedir, plot_dir, keys, months_nh, months_sh)

    # plot TC properties
    tc_assessment.plot_tc_properties(runid_info, storms, years, storedir, do_meanstate, do_variability, years_obs, plot_dir, keys, months_nh, months_sh, coupled=coupled)
